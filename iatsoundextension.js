	...
	function iatExtension(options)
	{
		...
		var iatObj =
		{
			...
			base_url : {//Where are your images at?
				image : '/implicit/user/yba/biat/images/'
			},
			...
			'If you make a mistake, a red <font color="#ff0000"><b>error</b></font> will appear. ' +
			...
			errorCorrection : false, //Should participants correct error responses?
			errorFBDuration : 300, //Duration of error feedback display (relevant only when errorCorrection is false)
			...
			fb_slight_Att1WithCatA_Att2WithCatB : 'Your data suggest a weak automatic preference for categoryB over categoryA.',
		...
		var proceedInput = !isTouch ? {handle:'space',on:'space'} : {handle:'space',on:'bottomTouch'};
		...
		*/
		...
		var basicTrialSets = {
			...
			att1left : createBasicTrialSet({side:'left', stimSet: 'att1'}),
			att1right : createBasicTrialSet({side:'right', stimSet: 'att1'}),
			att2left : createBasicTrialSet({side:'left', stimSet: 'att2'}),
			att2right : createBasicTrialSet({side:'right', stimSet: 'att2'}),
			//Four trials for the categories.
			cat1left : createBasicTrialSet({side:'left', stimSet: 'cat1'}),
			cat1right : createBasicTrialSet({side:'right', stimSet: 'cat1'}),
			cat2left : createBasicTrialSet({side:'left', stimSet: 'cat2'}),
			cat2right : createBasicTrialSet({side:'right', stimSet: 'cat2'}),
		};
		...
		API.addStimulusSets({
			...
			error : [{
				handle:'error', css:{color:'red','font-size':'4em'}, media: {word:'error'}, nolog:true
			}],
		...
		function getLayout(params)
		{
			...
			function buildContent(layout){
				...
				return isImage ? '<img src="' + content + '" style="max-width:100%;width:100%" />' : content;
			}
		...
		trialSequence.push({
			...
			stimuli : [
				{
					inherit : 'Default',
					media : isTouch ? {word : 'Touch the bottom green area to continue to the next task'} : {word : 'Press space to continue to the next task'}
				}
			]
		});
